...
- `send_photo_message(...)` e `authorize_chat_media(target_message_id)`: registram uma foto privada vinculada à conversa e a disponibilizam imediatamente aos participantes autorizados, sem triagem automática, revalidando bloqueio e estado das contas.
...
- `claim_profile_photo_moderation(...)` e `complete_profile_photo_moderation(...)`: RPCs exclusivas do `service_role` que processam somente a candidata do único espaço público de foto do perfil.
...
- `functions/`: Edge Functions e testes puros dos fluxos de aferição etária, álbum privado, push e moderação da foto de perfil.
...
- O bucket `chat-media` é privado, aceita somente JPEG/PNG/WebP de até 5 MB e usa caminhos vinculados ao remetente, conversa e chave idempotente. A mídia válida fica disponível aos participantes ativos sem triagem automática; bloqueio, denúncia, suspensão ou remoção revogam a leitura.
...
- A triagem gratuita `omni-moderation-latest` é exclusiva da candidata em `profile-photos`. Fotos do chat e imagens do álbum privado nunca são encaminhadas à OpenAI; continuam sujeitas à política, denúncia e moderação posterior. O worker persiste somente a decisão normalizada e não usa a classificação como verificação de idade.
...
- A migration `20260804180000_profile_photo_only_automation.sql` restringe a automação à única foto pública de perfil, deixa fotos do chat imediatamente disponíveis aos participantes e revoga do `service_role` o claim antigo do chat. Está aplicada e registrada no `Matcher Dev` desde 04/08/2026; as suítes hospedadas passaram com 25/25 e 35/35 asserções.
- A migration `20260804190000_schedule_private_workers.sql` agenda `notification-worker` e `profile-photo-moderation` a cada minuto com `pg_cron`/`pg_net` e bearer criptografado no Vault. Está aplicada e registrada no `Matcher Dev`; ambos os jobs concluíram e os endpoints responderam HTTP 200.
- A migration `20260804200000_profile_photo_storage_upload_protocol.sql` aceita `contentLength` na pré-checagem real do Storage e mantém a validação do `size` final e do limite de 5 MB. Está aplicada e registrada somente no `Matcher Dev`; a suíte hospedada de autorização passou com 40/40 asserções e o upload real no Samsung entrou corretamente como foto privada em análise.
- A migration `20260804210000_openai_profile_photo_moderation.sql` substitui erros específicos do Vision por códigos neutros, reinicia com segurança a fila pendente e mantém o lease/backoff. Está aplicada e registrada no `Matcher Dev`; a suíte hospedada atualizada concluiu 38/38 e a candidata real foi aprovada pelo novo worker.
- As Edge Functions `notification-worker` e `profile-photo-moderation` estão publicadas no `Matcher Dev`, com JWT legado desligado e autenticação própria pelo secret `WORKER_SHARED_SECRET`. O worker de foto usa `OPENAI_API_KEY` somente no Supabase e chama `/v1/moderations`; o worker antigo `chat-media-moderation` foi removido do remoto e do código ativo.
...
- Os onze arquivos pgTAP atuais declaram 445 asserções. As suítes atuais de chat e de push/moderação passaram no `Matcher Dev` com 25/25 e 38/38; a suíte da foto de perfil passou com 40/40, a de exclusão com 11/11, a de leases/reaper com 47/47 e a de autorização do álbum com 78/78. Para normalizar o runner hospedado, cada transação assume explicitamente `postgres`, pois a conexão chega como `cli_login_postgres` com `NOINHERIT`.
