...
- `create_private_album(...)`, `reserve_private_album_item(target_album_id, mime_type)` e `finalize_private_album_item(...)`: criam no máximo um álbum ativo, vinculam a reserva à geração exibida, reservam atomicamente até dez caminhos imutáveis e tornam uma imagem válida `available` sem aprovação prévia.
...
- Não usar `createSignedUrl` para álbum privado. A Edge Function `private-album-media` recebe apenas `item_id`, executa `authorize_private_album_item` com o JWT do usuário, valida novamente assinatura e dimensões reais de JPEG/PNG/WebP e transmite bytes pelo `service_role` com `Cache-Control: private, no-store, max-age=0`, sem redirect, URL persistente ou caminho no corpo/log. Imagens acima de 5 MB, 4.096 px por lado ou 16 milhões de pixels são recusadas.
...
- As três Edge Functions do álbum privado — `private-album-media`, `private-album-delete` e `private-album-cleanup` — estão publicadas no `Matcher Dev` desde 03/08/2026.
...
- Os sete arquivos pgTAP atuais declaram 314 asserções, todas aprovadas localmente e no projeto remoto em 03/08/2026. Para normalizar o runner hospedado, cada transação assume explicitamente `postgres`, pois a conexão chega como `cli_login_postgres` com `NOINHERIT`.
