...
- A migration `20260803130000_private_album_upload_reservation_leases.sql` adiciona idempotência explícita, TTL de 30 minutos, negação de upload/finalização tardios e reaper integrado ao worker com autorização exclusiva do `service_role`. Ela foi validada somente no ambiente local e **ainda não foi aplicada** ao `Matcher Dev` nem a produção.
- As três Edge Functions do álbum privado — `private-album-media`, `private-album-delete` e `private-album-cleanup` — estão publicadas no `Matcher Dev` desde 03/08/2026. `private-album-media` foi republicada no mesmo dia após alinhar a validação de identidade aos JWTs assimétricos atuais; seus 18 testes puros, o type-check e o bundle no Edge Runtime passaram antes da publicação. O endurecimento adicional do adaptador para as chaves atuais e para distinguir falha operacional de credencial inválida passou localmente, mas **ainda não foi republicado**.
